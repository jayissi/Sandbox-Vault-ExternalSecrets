...
> **Note:** When `VAULT_AUTO_UNSEAL=true`, Kubernetes Auth is configured during Vault initialization (see `hashicorp-vault-helm/`) for the `vault` service account, providing least-privilege CLI access for the auto-unseal sidecar.
