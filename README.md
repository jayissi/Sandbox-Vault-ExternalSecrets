...
- Unseal HashiCorp Vault based on the specified environment.
- Install External Secrets Operator to synchronize secrets from HashiCorp Vault to OpenShift.
- Populate HashiCorp Vault with demo secret.
- Configure External Secrets Operator to introduce HashiCorp Vault demo secret into OpenShift.
...
|-------------|-------------|
|  dev  | Deploy 1 HashiCorp Vault instance into "Dev" server mode. |
|  lab  | Deploy 1 HashiCorp Vault instance that auto initialize and unseal with auditing. |
|  prod | Deploy 3 HA HashiCorp Vault instances that auto initialize and unseal with auditing. |

   Set the local variable **VAULT_ENV** to either `dev`, `lab`, or `prod`.     
   This determines if HashiCorp Vault will be initialized and unsealed during the deployment.
   ```bash
...
### **Trust But Verify**
   ...
   - Ensure that the HashiCorp Vault pods are running and unsealed, if needed.
...
- **How it works**: The `Makefile` automates the deployment of HashiCorp Vault using Helm. HashiCorp Vault is deployed into the OpenShift cluster, and the necessary configurations are applied for both the lab and production environments.
...
1. **Vault is deployed**: Using the Helm chart, HashiCorp Vault is deployed to your OpenShift cluster.
2. **Vault is unsealed**: After deployment, HashiCorp Vault is unsealed automatically based on the environment (`lab` or `prod`).
3. **External Secrets Operator is installed**: External Secrets Operator is installed via Helm to manage the synchronization of secrets from HashiCorp Vault to OpenShift.
4. **Demo secrets are created in Vault**: HashiCorp Vault is populated with demo secrets that represent real-world credentials and data.
5. **Secrets are synchronized to OpenShift**: External Secrets Operator automatically syncs HashiCorp Vault secrets as Kubernetes Secrets in OpenShift, making them available for use by OpenShift.
