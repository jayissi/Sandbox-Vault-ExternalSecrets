...
login_with_k8s_auth() {
    ...
    vault_token=$(echo "$vault_response" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('auth',{}).get('client_token',''))" 2>/dev/null)
...
check_and_unseal() {
    ...
    while IFS= read -r key; do
        if [[ -n "$key" ]]; then
            ...
            local still_sealed
            still_sealed=$(echo "$check" | python3 -c "import sys,json; print(json.load(sys.stdin).get('sealed', True))" 2>/dev/null || echo "true")
